...
} else if (artifactKind.includes("failure message") || artifactKind.includes("recovery")) {
  prototype = `<div class="cards"><div class="card"><b>Cause</b><p>Name what failed without blaming the person.</p></div><div class="card"><b>Impact</b><p>State what is unavailable and what remains safe.</p></div><div class="card"><b>Recovery</b><p>Give one reversible next action and say what it cannot guarantee.</p></div></div>`;
} else if (artifactKind.includes("requirement")) {
  prototype = `<div class="cards"><div class="card"><b>Original language</b><p>Preserve the ambiguous statement before translating it.</p></div><div class="card"><b>Formal constraint</b><p>Make the interpretation explicit and testable.</p></div><div class="card"><b>Loss introduced</b><p>Record what the formal representation leaves unresolved.</p></div></div>`;
} else if (artifactKind.includes("documentation drift") || artifactKind.includes("comparison")) {
  prototype = `<div class="cards"><div class="card"><b>Claimed behavior</b><p>State the documented promise in one inspectable sentence.</p></div><div class="card"><b>Observable behavior</b><p>State only what the repository can verify.</p></div><div class="card"><b>Unresolved difference</b><p>Do not turn a mismatch into a claim about consequences.</p></div></div>`;
} else if (artifactKind.includes("disagreement") || artifactKind.includes("evidence boundary")) {
  prototype = `<div class="cards"><div class="card"><b>Unresolved question</b><p>${escapeHtml(experiment.philosophicalTension ?? "What does this artifact make visible, and what remains unknowable?")}</p></div><div class="card"><b>Concrete response</b><p>${escapeHtml(experiment.test)}</p></div><div class="card"><b>Evidence boundary</b><p>${escapeHtml(experiment.missingEvidence)}</p></div></div>`;
}
